...
<script type="text/javascript">
	...
	{ $('#commentBox').on('keyup', function() {
		...
		$(".btnList").click(function()
		{
			...
			$(location).attr("href", "freeboardlist.action?"); // + pagenum
		});

		/* 게시물 추천 버튼 클릭 */
		$(".btnRec").click(function()
		{
			$(location).attr("href","freeboard_rec.action?check=1&post_code="+ $(this).val());
		});
		...
		{
			$(location).attr("href","freeboard_rec.action?check=2&post_code="+ $(this).val());
		});
		...
		$(".btnDelete").click(function()
		{
			...
			if(result)
			{
				location.replace("freeboard_delete.action?post_code=" + $(this).val());
			}
		});
		...
		{
			$(location).attr("href", "freeboard_update_form.action?post_code=" + $(this).val());
		});
		
		/* 댓글 삭제 버튼 클릭 */
		$(".btndelCmt").click(function()
		{
			$(location).attr("href", "deletecomment.action?post_code=" + $(this).attr("value1") + "&comment_code=" + $(this).attr("value2"));
		});
...
<div class="wrapper">
			...
			<div class="main">
				<form action="" name="myForm">
					<table id="table1" class="table">
						<tr>
							<th colspan="8" style="background-color: #ffffff;">${article.title }</th>
						</tr>
						<tr>
							<th>작성자</th>
						...
						<tr>
							...
							<c:choose>
								<c:when test="${sessionScope.userid == article.user_id }">
  ...
  <path
				...
				<!-- Modal -->
					<form action="freeboardreport.action" method="post" onsubmit="return chk();">
						<input type="hidden" name="post_code" value="${article.post_code }"/>
				...
				<form action="addcomment.action" name="myForm">
					<table id="commentWrite" class="table">
						<tr>
							<td colspan="2">
								<input type="hidden" id="post_code" name="post_code" value="${article.post_code }"/>
								<textarea id="commentBox" name="commentBox"	class="form-control" placeholder="댓글을 입력하세요."></textarea>
							</td>
						</tr>
						<tr>
							<td>
