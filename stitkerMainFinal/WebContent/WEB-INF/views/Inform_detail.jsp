	...
	$(function()
	{
		...
		$(".btnRec").click(
	            function()
	            {
	               $(location).attr(
	                     "href",
	                     "inform_rec.action?check=1&post_code="
	                           + $(this).val());
	            });
		 // 비추천 버튼 클릭 시 비추천 수 증가
	      $(".btnNotRec").click(
          function()
          {
             $(location).attr(
                   "href",
                   "inform_rec.action?check=2&post_code="
                         + $(this).val());
          });
	      ...
	      $(".cmtUpdateBtn").click(function() {
				...
				//alert(comment_code);
				$.post("getinformcomm.action?comment_code="+comment_code
				, function(cmtList)
				{
					$("#commentBox").val(cmtList);
					$("#commBtn").html("수정");
					$("#commForm").attr({"action":"modifyinformcomm.action?comment_code="+comment_code});
			...
			$(".cmtDelBtn").click(function()
			{
				...
				if (confirm("현재 선택한 데이터를 정말 삭제하시겠습니까?"))
				{
					$(location).attr("href", "informcmtdelete.action?comment_code=" + $(this).val());
				} 
			});  
...
<div class="wrapper">
 ...
 <div class="main-content text-center"><br>
	...
	<div class="main">
		<form action="informupdateform.action" method="get" name="myForm">
		...
		<input type="hidden" name="post_code" value="${detail.post_code}">
			...
			<tr>
				...
				<td colspan="4" style="text-align: right; border-bottom: #ffffff;">
			        ...
			        <c:when test="${sessionScope.admin==null && sessionScope.code != null && detail.user_code==sessionScope.code}">
		...
		<!-- Modal -->
             <form action="informreport.action" method="post" onsubmit="return chk()">
		...
		<table id="table2" class="table">
				...
				<tr>
			 		...
			 		<c:choose>
					<c:when test="${sessionScope.code!=null && sessionScope.code==comment.user_code}">
                    	...
                    	<td style="width: 5%">
                    		<!-- 댓글 삭제 -->
                             <button id="deleteComm" style="color: black;" class="btn btn-link btn-sm cmtDelBtn"
                             value="${comment.comment_code }">
			        ...
			        <c:when test="${sessionScope.code!=null && sessionScope.admin==null && sessionScope.code!=comment.user_code}">
			        	<td id="comment" style="width: 65%;">${comment.comments }</td>
			       		<td colspan="3">${comment.write_date }</td>
			        </c:when>
		...
		<form action="infocmtinsert.action" method="post" name="myForm" id="commForm">
		<input type="hidden" name="code" value="${sessionScope.code}">
		<input type="hidden" name="post_code" value="${detail.post_code}">
