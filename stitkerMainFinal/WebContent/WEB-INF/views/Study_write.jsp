...
<script type="text/javascript">
    ...
    {
    	// 스터디 개설 자격 체크(안되면 메인으로 돌아감.)
    	$.post("checkstudyinsert.action"
		, function(msg)
		{
    		if (msg != '' && msg != null && msg != ' ' && msg.length > 2)
    		{	
				alert(msg);
				$(location).attr('href', 'studyit.action');
    		}	
...
<div class="wrapper">
 ...
 <div class=" main main-content text-center"><br>
	...
	<div class="main">
	<form action="studyinsert.action" method="get" name="studyForm" >
	...
	<table id="table1" class="table table-striped table-bordered">
		<tr>
			<th>스터디 이름</th>
			<td><input type="text" maxlength="60" name="study_name" class="form-control" id="study_name" required></td>
		</tr>
		<tr>
			<th>스터디의 목적 및 목표</th>
			<td>
				<textarea rows="2" cols="40" name="study_desc" id="purpose" class="form-control" required></textarea>
		...
		<tr>
			<th>시작 날짜</th>
			<td>
				<input type="text" id="startDate" name="start_date" class="form-control" placeholder="스터디 시작일" required>
			</td>
		</tr>
		<tr>
			<th>종료 날짜</th>
			<td>
				<input type="text" id="endDate" name="end_date" class="form-control" placeholder="스터디 종료일" required >
		...
		<tr>
			<th>최소인원수</th>
			<td>
				<select name="min_mem_code" id="minMem" class="form-control"  required>
					<option value="">선택</option>
					 <c:forEach var="memnum" items="${mnList }">
						 <option value="${memnum.memnum_code }">
						 	${memnum.memnum}
						 </option>
					 </c:forEach>
				</select>
		...
		<tr>
			<th>참여최소등급</th>
			<td>
				<select name="min_rank" id="grade" class="form-control"  required>
					<option value="">선택</option>
	...
	<div id="footer">
		<button type="submit" id="sendBtn" class="btn btn-outline-primary" onclick="sendIt()">등록</button>
		<button type="button" class="btn btn-outline-primary">재입력</button>
