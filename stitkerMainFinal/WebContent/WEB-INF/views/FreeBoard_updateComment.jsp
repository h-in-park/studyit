			...
			function()
			{
				...
				$(".btnRec").click(
						function()
						{
							$(location).attr(
									"href",
									"freeboard_rec.action?check=1&post_code="
											+ $(this).val());
						});
				...
				{
					$(location).attr("href", "freeboard_delete.action?post_code=" + $(this).val());
				});
...
<body>
	<div class="wrapper">
		...
		<div class="main-content text-center"><br>
			...
			<div class="main"><br>
				<form action="updatecomment.action" name="myForm">
					<table id="table1" class="table">
						...
						<tr>
							<td colspan="8" style="height: 300px;">${article.content }</td>
						</tr>
						...
						<tr>
							<td colspan="4" style="border-bottom: #ffffff;"><input
								type="button" value="리스트"
								class="btn btn-outline-primary btn-sm btnList"></td>
							<c:choose>
								<c:when test="${sessionScope.userid == article.user_id }">
									<td colspan="4"	style="text-align: right; border-bottom: #ffffff;">
										<button type="button" value="${article.post_code }" 
													class="btn btn-outline-primary btn-sm btnUpdate">수정</button>
										<button type="button" value="${article.post_code }" 
													class="btn btn-outline-primary btn-sm btnDelete">삭제</button>
									</td>
								</c:when>
								<c:otherwise>
									<td colspan="4"
										style="text-align: right; border-bottom: #ffffff;"><input
										type="button" value="신고" class="btn btn-outline-danger btn-sm">
									</td>
								</c:otherwise>
							</c:choose>
  ...
  <path
						...
						<c:forEach var="comment" items="${cmtList }">
							<tr>
								...
								<td>
									<c:choose>
										<c:when test="${sessionScope.code!=null && sessionScope.userid==comment.id }">
											...
											<a href="" style="color: black;"><!-- 댓글 수정 버튼 -->
					            ...
					            <td>
									<c:choose>
										<c:when test="${sessionScope.code!=null && sessionScope.userid==comment.id }">
											<a href="deletecomment.action" style="color: black;"><!-- 댓글 삭제 버튼 -->
					...
					<table id="commentWrite" class="table">
						<tr>
							<td colspan="2">
								<textarea id="commentBox" name="commentBox"	class="form-control" placeholder="댓글을 입력하세요.">${comments }</textarea>
							</td>
						</tr>
						<tr>
							<td>
							...
							<td>
								<button type="submit" 
									class="btn btn-outline-primary btn-sm btn2 btnCmt">수정</button>
							</td>
						</tr>
					</table>
