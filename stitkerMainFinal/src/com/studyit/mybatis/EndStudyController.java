...
@Controller
public class EndStudyController
{
	 @Autowired
	 private SqlSession sqlSession;
	 
	 @RequestMapping(value = "/endstudy.action", method = RequestMethod.GET)
	  public String studentList(HttpServletRequest request, ModelMap model)
	  {
		 // 요청 주소 
		 String result = null;
		 result = "WEB-INF/views/Mypage_endStudy.jsp";
		 ...
		 IEndstudyDAO dao = sqlSession.getMapper(IEndstudyDAO.class);
		...
		// 페이징
		String listUrl = "endstudy.action?studycode="+ studycode;
		...
		model.addAttribute("articleUrl", articleUrl);
	     ...
	     // 스터디 후기 작성했는지 확인 
		 model.addAttribute("reviewCheck",dao.reviewCheck(parti_code));	
		
		 // 스터디 평가했는지 확인
		 model.addAttribute("saCheck", dao.saCheck(parti_code));
		 
		 // 리스트 출력
	     model.addAttribute("endlist", dao.list(studycode, id));
	     model.addAttribute("weekday", dao.weekday(studycode));
	     model.addAttribute("endcontents",contents.contentslist(studycode, start2, end2));
	     ...
	     //model.addAttribute("score", dao.score(parti_code));
	     request.setAttribute("grade", dao.grade(parti_code));
	     
	     return result;
	     //return "WEB-INF/views/Mypage_endStudy.jsp";
	   }
	 
		// 스터디 점수 입력
	 	@RequestMapping(value = "/studyassess.action", method = RequestMethod.GET)
		public String contentInsert(HttpServletRequest request, EndstudyDTO dto,ModelMap model)
		{
	 		HttpSession session = request.getSession();
	 		IEndstudyDAO dao = sqlSession.getMapper(IEndstudyDAO.class);
	 		String id = (String)session.getAttribute("userid");
			...
			dao.studyassess(dto);
 			...
 			return "redirect:endstudy.action?studycode=" +studycode;
